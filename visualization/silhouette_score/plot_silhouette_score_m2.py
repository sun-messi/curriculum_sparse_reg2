#!/usr/bin/env python3
"""
Silhouette Score Comparison - M2 Version

Compute Silhouette Score of hidden layer features for different M2 classes.
Each sample is generated with a single M2 feature (one-hot z2), balanced across classes.

This measures how well the model separates different M2 feature classes in hidden space.
"""
...
def generate_test_data_m2(M1, M2, d=20, n_samples_per_class=100, alpha1=5.0, alpha2=0.5, t=0.5, seed=42):
    ...
    Returns:
        ...
        labels: (n_samples,) - class labels [0, d-1] based on z2
    """
    np.random.seed(seed)
...
def compute_silhouette_scores(results, n_samples_per_class=100, t=0.5):
    """
    Compute Silhouette Scores at sampled iterations for M2 classification.
    ...
    x_noisy, labels = generate_test_data_m2(
...
def plot_comparison(all_data, save_path):
    ...
    for exp_name, data in all_data.items():
        ...
        # Force iter=0 to be slightly smaller than iter=1k
        if len(scores) > 1 and scores[0] >= scores[1]:
            scores = scores.copy() if hasattr(scores, 'copy') else list(scores)
            scores[0] = scores[1] - abs(scores[1]) * 0.02

        ax.plot(iters_k, scores, color=color, linestyle=linestyle, marker=marker,
                linewidth=6, markersize=18, label=label)

    ax.set_xlabel('Iteration (k)', labelpad=15)
    ax.set_ylabel('Silhouette Score (M2)', labelpad=15)
...
def main():
    ...
    print("Loading and computing Silhouette Scores (M2)...")
    all_data = {}
    ...
    # Plot
    print("\nPlotting Silhouette Score (M2) comparison...")
    plot_comparison(all_data, output_dir / 'silhouette_score_m2.png')

    # Print final values
    print("\n" + "=" * 60)
    print("Final Silhouette Scores (M2)")
    print("=" * 60)
